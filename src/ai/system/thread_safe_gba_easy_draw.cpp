...
static void RenderBGScanline(
	struct EmeraldAddresses* eme, int bgNum, uint16_t control,
	uint16_t hoffs, uint16_t voffs, int lineNum, uint16_t *line
){
    ...
    uint16_t *pal = (uint16_t *)eme->PLTT;
...
static inline uint32_t getBgX(struct EmeraldAddresses* eme, int bgNumber)
{
    if (bgNumber == 2)
    {
        return REG_BG2X;
...
static inline uint32_t getBgY(struct EmeraldAddresses* eme, int bgNumber)
{
    if (bgNumber == 2)
    {
        return REG_BG2Y;
...
static inline uint16_t getBgPA(struct EmeraldAddresses* eme, int bgNumber)
{
    if (bgNumber == 2)
    {
        return REG_BG2PA;
...
static inline uint16_t getBgPB(struct EmeraldAddresses* eme, int bgNumber)
{
    if (bgNumber == 2)
    {
        return REG_BG2PB;
...
static inline uint16_t getBgPC(struct EmeraldAddresses* eme, int bgNumber)
{
    if (bgNumber == 2)
    {
        return REG_BG2PC;
...
static inline uint16_t getBgPD(struct EmeraldAddresses* eme, int bgNumber)
{
    if (bgNumber == 2)
    {
        return REG_BG2PD;
...
static void RenderRotScaleBGScanline(
	struct EmeraldAddresses* eme, int bgNum, uint16_t control,
	uint16_t x, uint16_t y, int lineNum, uint16_t *line
) {
    ...
    s16 pa = getBgPA(eme, bgNum);
    s16 pb = getBgPB(eme, bgNum);
    s16 pc = getBgPC(eme, bgNum);
    s16 pd = getBgPD(eme, bgNum);
    ...
    s32 currentX = getBgX(eme, bgNum);
    s32 currentY = getBgY(eme, bgNum);
...
static uint16_t alphaBlendColor(struct EmeraldAddresses* eme, uint16_t targetA, uint16_t targetB)
{
...
static uint16_t alphaBrightnessIncrease(struct EmeraldAddresses* eme, uint16_t targetA)
{
...
static uint16_t alphaBrightnessDecrease(struct EmeraldAddresses* eme, uint16_t targetA)
{
    unsigned int evy = (REG_BLDY & 0x1F);
...
static bool alphaBlendSelectTargetB(
	struct EmeraldAddresses* eme, struct scanlineData* scanline, uint16_t* colorOutput,
...
static void DrawSprites(struct EmeraldAddresses* eme, struct scanlineData* scanline, uint16_t vcount, bool windowsEnabled)
{
    int i;
    unsigned int x;
    unsigned int y;
    ...
    for (i = 127; i >= 0; i--)
    {
        struct OamData *oam = &((struct OamData *)eme->OAM)[i];
        unsigned int width;
        unsigned int height;
        uint16_t *pixels;
        ...
        if (isAffine)
        {
            ...
            struct OamData *oam1 = &((struct OamData *)eme->OAM)[matrixNum];
            struct OamData *oam2 = &((struct OamData *)eme->OAM)[matrixNum + 1];
            struct OamData *oam3 = &((struct OamData *)eme->OAM)[matrixNum + 2];
            struct OamData *oam4 = &((struct OamData *)eme->OAM)[matrixNum + 3];
        ...
        if (vcount >= (y - half_height) && vcount < (y + half_height))
        {
            ...
            for (int local_x = -half_width; local_x <= half_width; local_x++)
            {
                ...
                uint16_t *palette = (uint16_t *)(eme->PLTT + 0x200);
                int local_mosaicX;
                int tex_x;
                int tex_y;
                ...
                if (pixel != 0)
                {
                    ...
                    if (global_x < DISPLAY_WIDTH && global_x >= 0)
                    {
                        ...
                        if ((blendMode == 1 && REG_BLDCNT & BLDCNT_TGT1_OBJ && winShouldBlendPixel) || isSemiTransparent)
                        {
                            ...
                            uint16_t targetB = 0;
                            if (alphaBlendSelectTargetB(eme, scanline, &targetB, oam->priority, 0, global_x, false))
                            {
                                color = alphaBlendColor(eme, targetA, targetB);
                            }
                        }
                        else if (REG_BLDCNT & BLDCNT_TGT1_OBJ && winShouldBlendPixel)
                        {
                            ...
                            {
                            case 2:
                                color = alphaBrightnessIncrease(eme, color);
                                break;
                            case 3:
                                color = alphaBrightnessDecrease(eme, color);
                                break;
                            }
                        }
...
static void DrawScanline(struct EmeraldAddresses* eme, uint16_t *pixels, uint16_t vcount)
{
    ...
    case 0:
        ...
        for (bgnum = 3; bgnum >= 0; bgnum--)
        {
            if (isbgEnabled(bgnum))
            {
                ...
                RenderBGScanline(eme, bgnum, scanline.bgcnts[bgnum], bghoffs, bgvoffs, vcount, scanline.layers[bgnum]);
            }
        }
    ...
    case 1:
        ...
        if (isbgEnabled(bgnum))
        {
            RenderRotScaleBGScanline(eme, bgnum, scanline.bgcnts[bgnum], REG_BG2X, REG_BG2Y, vcount, scanline.layers[bgnum]);
        }
    ...
    if (REG_DISPCNT & DISPCNT_OBJ_ON)
        DrawSprites(eme, &scanline, vcount, windowsEnabled);
                ...
                for (xpos = 0; xpos < DISPLAY_WIDTH; xpos++)
                {
                    ...
                    if (blendMode != 0 && REG_BLDCNT & (1 << bgnum) && winEffectEnable)
                    {
                        ...
                        case 1:
                            ...
                            if (alphaBlendSelectTargetB(eme, &scanline, &targetB, prnum, prsub+1, xpos, isSpriteBlendingEnabled))
                            {
                                color = alphaBlendColor(eme, targetA, targetB);
                            }
                            break;
                        case 2:
                            color = alphaBrightnessIncrease(eme, targetA);
                            break;
                        case 3:
                            color = alphaBrightnessDecrease(eme, targetA);
                            break;
                        }
                    }
...
void DrawFrame(uint16_t *pixels, struct EmeraldAddresses* eme)
{
    int i;
    int j;
    ...
    for (i = 0; i < DISPLAY_HEIGHT; i++)
    {
        ...
        unsigned int blendMode = (REG_BLDCNT >> 6) & 3;
        uint16_t backdropColor = *(uint16_t *)eme->PLTT;
        if (REG_BLDCNT & BLDCNT_TGT1_BD)
        {
            switch (blendMode)
            {
            case 2:
                backdropColor = alphaBrightnessIncrease(eme, backdropColor);
                break;
            case 3:
                backdropColor = alphaBrightnessDecrease(eme, backdropColor);
                break;
            }
        }

		// seems to not be needed but causes a crash? weird.
		//memsetu16(&pixels[i * DISPLAY_WIDTH], backdropColor, DISPLAY_WIDTH);
        DrawScanline(eme, &pixels[i * DISPLAY_WIDTH], i);
